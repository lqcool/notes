...
>
> - 如果用以前的方法肯定不行，因为时间可能是 变化的，例如2015变为2016，这个时候写死的正则表达式就不可以是使用了，如"2015-12-25".replace(/\d{3}-\d{2}-\d{2}/g,"12/25/2015");这是一种死的写法。
>
> - 比较活用的就是使用分组，并且使用$来访问捕获的分组，上面的代码就成了这个样子：
>
>   "2015-12-25".replace(/\(d{3})-(\d{2})-(\d{2})/g,"$2\$3\$1")
>
> **忽略分组**
>
